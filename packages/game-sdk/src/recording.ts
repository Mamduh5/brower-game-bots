...
/** Ordered down/up edges retain overlapping holds. Only mouse samples are coalesced by the native recorder. */
export function recordingToProfile(raw: readonly RecordedEvent[], name = "My recording"): DesktopProfile {
  ...
  if (raw.length > 2100) throw new Error("Recording event limit exceeded");
  let previous = 0;
