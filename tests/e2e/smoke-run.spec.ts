...
test("cli smoke run completes one tester cycle and writes report artifacts", async () => {
  const repoRoot = fileURLToPath(new URL("../..", import.meta.url));
  const tempDir = await mkdtemp(path.join(os.tmpdir(), "game-bots-e2e-tester-"));
  const sqlitePath = path.join(tempDir, "run.sqlite");
  const artifactsPath = path.join(tempDir, "artifacts");
  const overrideConfigPath = path.join(tempDir, "e2e-tester.override.yaml");
  ...
  const { stdout, stderr } = await execFileAsync(
    process.execPath,
    [path.join(repoRoot, "apps", "bot-cli", "dist", "main.js"), "run-tester"],
    {
      cwd: repoRoot,
      env: {
        ...process.env,
  ...
  expect(stderr).toBe("");
  expect(stdout).toContain("Completed tester run");
  expect(artifactRuns.length).toBeGreaterThan(0);
});
