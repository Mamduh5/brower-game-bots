...
- The learner retrieves at most 12 nearby demonstrated transitions. It scans at most four distinct target patches per decision. A 9×9 RGB patch uses normalized correlation and colour error at three scales (0.8, 1, 1.25), coarse search and local refinement. A target needs score ≥0.78 and a ≥0.045 margin over another spatially distinct match. Duplicate-looking objects can therefore cause a help request.
...
- Clicks use the newly matched target position. Directional controls with annotated targets also depend on target position relative to demonstrated positions. The controller receives bounded actions only; historical click coordinates are never used as a fallback.
- Ranking combines visual similarity, target evidence, smoothed success/failure statistics and a recent-repeat penalty. Displayed confidence is a **heuristic evidence score, not a calibrated probability**. Default minimum confidence is 0.62. Weak accepted actions are shortened to 300 ms; the normal maximum is 800 ms (configurable 100–1,000 ms).
- The result is compared with the demonstrated resulting appearance and, when annotated, the direction of target motion. Matching positive evidence strengthens an action; contrary/failed transitions weaken it. Nearly unchanged scenes are neutral and carry a smaller penalty. Unexplained animation is uncertain and earns no success credit. Every dispatched skill gets a result update; observation-only waits do not.
...
No Roblox, real desktop/game, native fixture, browser/GUI E2E, live provider or long-soak testing was performed for this implementation. See [design/research](adr/0007-local-learned-desktop.md) and [code validation](local-learned-validation.md).
