...
2. Optionally choose **Capture target preview**. This requests focus and captures the visible client area without sending gameplay input. Return to the GUI and click a point on the preview to fill the selected Click/Move action's X/Y fields.
...
6. Choose **Start Bot** and allow the target to stay foreground. **Pause Bot** releases held input; **Resume Bot** explicitly requests focus and takes a fresh observation. **Stop Bot / cancel start** terminates the run or pending start. Recording uses separate controls and never starts playback automatically.
...
Choose once, N loops, or until stopped, with a separate delay between loops. Legacy profiles without loop settings repeat in order until either safety limit is reached. Both limits are mandatory safety bounds (defaults: 100 attempts, 60 seconds). Limits include start delay, loop delay and paused time. Recorded profiles use larger default safety bounds. Allowed range: 1–10,000 action attempts, 1–3,600 seconds, interval 100–60,000 ms, and start delay 0–60 seconds. The interval is an additional delay after each action, not a promised click frequency: capture/input costs and press duration also count. In fixed-interval mode, maximum requested sequence rate is 10 actions/sec; real screenshot-driven throughput is lower. Recorded mode instead schedules event delays and captures screenshots at loop boundaries. Attempts that were interrupted count toward the limit and are not automatically retried.
...
**Roblox was not launched or validated during implementation.** The general nine-action control profile was validated against the native fixture. No public farming or competitive automation was performed. No process injection, executable modification, memory reading, game-private hooks, packet manipulation, anti-cheat bypass or evasion exists. Recording uses ordinary Windows low-level desktop hooks. The fixture evidence above predates recording; [recording validation](recording-validation.md) is code-level only.
...
- **Run stops on held-input limit:** Replace long key-down sequences with shorter hold/release episodes. A held key during a long interval still counts toward the configured hold limit (5.5 seconds by default; up to 60 seconds for recordings).
